...
@implementation GoodsImageListView
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    if (_imageHeightBlock) {
        _imageHeightBlock([self makeAllDetailsImage:imageArray]);
    }
}

- (CGFloat)makeAllDetailsImage:(NSArray *)imageArray{
    float height = 0;
    for (GoodsImagListModel  *imageModel in imageArray) {
